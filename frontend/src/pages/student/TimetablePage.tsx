...
export default function TimetablePage() {
  ...
  const handleSetReminder = async (item: TimetableItem) => {
    try {
      ...
      if (res.ok) {
        setReminderToast(`⏰ Reminder set! You will be notified 10 mins before ${item.subject_name}.`);
    ...
    <div className="w-full h-full overflow-y-auto bg-[#F8FAFC] dark:bg-[#0B0F19] text-[#1E293B] dark:text-[#F8FAFC] p-3 sm:p-6 md:p-8 font-body transition-colors duration-300">
      <div className="w-full max-w-[1600px] mx-auto space-y-6">
        ...
        <AnimatePresence>
          {reminderToast && (
            ...
            >
              ...
              <button onClick={() => setReminderToast(null)}>✕</button>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="bg-white dark:bg-[#111827] p-5 sm:p-6 rounded-2xl border border-[#E2E8F0] dark:border-[#1E293B] shadow-xs space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#0E2A6D] to-[#1E4DB7] text-white flex items-center justify-center shadow-md border border-[#D9A441]/30 shrink-0">
                <CalendarIcon size={30} strokeWidth={1.75} />
              </div>
              <div>
                ...
                <p className="text-xs sm:text-sm text-[#64748B] dark:text-[#94A3B8] mt-0.5">
                  {todayData.current_date} • {todayData.current_day} ({todayData.current_time})
                </p>
              </div>
            </div>
          ...
          {/* Ongoing & Next Class Highlight Banner */}
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-r from-[#1E4DB7]/10 to-[#0E2A6D]/10 border-2 border-[#1E4DB7] dark:border-[#60A5FA] flex items-center justify-between shadow-xs">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-extrabold uppercase tracking-wider text-[#1E4DB7] dark:text-[#60A5FA]">
                      Ongoing Class Right Now
                    </span>
                  </div>
                  <h3 className="font-heading font-bold text-base text-[#0E2A6D] dark:text-white">
                    {ongoingClass.subject_name}
                  </h3>
                  <div className="flex items-center gap-3 text-xs text-[#64748B] font-semibold">
                    <span className="flex items-center gap-1"><UserCheck size={14} /> {ongoingClass.faculty_name}</span>
                    <span className="flex items-center gap-1"><MapPin size={14} /> {ongoingClass.classroom}</span>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <span className="text-xs font-bold text-[#1E4DB7] dark:text-[#60A5FA] block">
            ...
            {nextClass && (
              <div className="p-4 rounded-2xl bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] flex items-center justify-between shadow-xs">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3 text-xs text-[#64748B] font-semibold">
                    <span className="flex items-center gap-1"><UserCheck size={14} /> {nextClass.faculty_name}</span>
                    <span className="flex items-center gap-1"><MapPin size={14} /> {nextClass.classroom}</span>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  ...
                  <button
                    ...
                    className="mt-1 px-2.5 py-1 rounded-lg bg-[#D9A441]/20 text-[#0E2A6D] dark:text-[#D9A441] text-[10px] font-bold border border-[#D9A441]/30 hover:bg-[#D9A441]/30 transition"
                  >
                    + Set Reminder
                  </button>
                </div>
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-xs text-[#64748B] font-semibold">
                        <span className="flex items-center gap-1"><UserCheck size={14} /> {item.faculty_name}</span>
                        <span className="flex items-center gap-1"><MapPin size={14} /> {item.classroom}</span>
                        <span className="flex items-center gap-1"><Clock size={14} /> {item.start_time} - {item.end_time}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {item.status === 'Upcoming' && (
                        <button
                          ...
                          className="px-3.5 py-2 rounded-xl bg-[#F8FAFC] dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] text-xs font-bold text-[#0E2A6D] dark:text-[#D9A441] hover:border-[#D9A441] transition flex items-center gap-1.5"
                        >
        ...
        {/* ========================================================================= */}
        {/* MODE 2: WEEKLY TIMETABLE MATRIX & DAY SELECTOR                            */}
        {/* ========================================================================= */}
        {viewMode === 'week' && (
              ...
              {(weeklyData[selectedDay] || []).map((item) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-[#1E4DB7]">{item.start_time} - {item.end_time}</span>
                    <span
                  ...
                  <h4 className="font-heading font-bold text-base text-[#0E2A6D] dark:text-white">
                    {item.subject_name}
                  </h4>
                  <div className="text-xs text-[#64748B] space-y-1">
                    <p>👨‍🏫 {item.faculty_name}</p>
                    <p>🏫 {item.classroom}</p>
                  </div>
                </div>
              ))}
        ...
        {viewMode === 'calendar' && (
          <div className="bg-white dark:bg-[#111827] p-6 rounded-2xl border border-[#E2E8F0] dark:border-[#1E293B] shadow-xs space-y-6">
            <h2 className="font-heading font-bold text-xl text-[#0E2A6D] dark:text-white flex items-center gap-2">
              <CalendarIcon size={22} className="text-[#D9A441]" />
              Academic Calendar & Event Highlights
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {calendarEvents.map((ev, idx) => (
                ...
                >
                  ...
                  <p className="text-xs text-[#64748B]">📅 {ev.date}</p>
                </div>
              ))}
            </div>
          </div>
